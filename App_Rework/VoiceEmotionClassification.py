...
def recording():
    ...
    start_recording = f"Start Recording in {Recording.TIMEOUT} seconds"
...
def update_recording_text():
    ...
    recording_text_widget.config(state=tk.NORMAL)  # Enable the widget for editing
    recording_text_widget.insert(tk.END, "Recording finished")  # Insert the new result
    recording_text_widget.config(state=tk.DISABLED)  # Disable editing
...
def create_VoiceEmotionClassification_window():
    ...
    start_recording_widget = tk.Text(root, wrap=tk.WORD, state=tk.DISABLED, height=2, bg="yellow")  # Adjust the height as needed
    start_recording_widget.place(relx=0.5, rely=0.25, anchor=tk.CENTER)

    recording_text_widget = tk.Text(root, wrap=tk.WORD, state=tk.DISABLED, height=2, bg="yellow")  # Adjust the height as needed
    ...
    analyze_text_widget = tk.Text(root, wrap=tk.WORD, state=tk.DISABLED, height=4, bg="yellow")  # Adjust the height as needed
